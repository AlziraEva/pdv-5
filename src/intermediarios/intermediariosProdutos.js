...
const validarPedido = async(req, res, next) =>{
    ...
    try{
        for(const produto of pedido_produtos){
            const{produto_id, qunatidade_produto} = produto;
            ...
            if(produtoExiste.qunatidade_produto < qunatidade_produto){
                return res.status(400).json({ mensagem: 'Qyantidade em estoque insuficiente.'})
            }
        }
